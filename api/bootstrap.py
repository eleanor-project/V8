...
def load_constitutional_config(path_override: Optional[str] = None) -> Dict[str, Any]:
    ...
    config_path = (
        path_override or os.getenv("CONSTITUTIONAL_CONFIG_PATH") or "governance/constitutional.yaml"
    )
...
def _parse_critic_bindings(raw: Optional[str]) -> Dict[str, str]:
    ...
    try:
        ...
        logger.warning(
            "Failed to parse CRITIC_MODEL_BINDINGS as JSON; falling back to comma parsing"
        )
...
def bind_critic_models(engine, bindings: Optional[Dict[str, str]] = None) -> None:
    ...
    for critic, adapter_name in mapping.items():
        ...
        if adapter is None:
            logger.warning(
                f"Critic binding skipped: adapter '{adapter_name}' not found for critic '{critic}'"
            )
            continue
...
def build_engine(constitution: Optional[Dict[str, Any]] = None) -> Any:
    ...
    evidence_path = (
        os.getenv("EVIDENCE_PATH") or os.getenv("ELEANOR_EVIDENCE_PATH") or "evidence.jsonl"
    )
...
async def evaluate_opa(
    callback, payload: Dict[str, Any], fallback_strategy: Optional[str] = None
) -> Dict[str, Any]:
    """
    Call OPA callback regardless of sync/async implementation.
    fallback_strategy: "escalate" (default), "deny", or "allow" on failure.
    """
    fallback = (fallback_strategy or os.getenv("OPA_FAIL_STRATEGY") or "escalate").lower()
    ...
    if callback is None:
        if fallback == "allow":
            return {
                "allow": True,
                "escalate": False,
                "failures": [{"policy": "opa_missing", "reason": "No OPA configured"}],
            }
        if fallback == "deny":
            return {
                "allow": False,
    ...
    try:
        ...
        return result or {
            "allow": False,
            "failures": [{"policy": "empty", "reason": "No result returned"}],
        }
