...
from engine.engine import EleanorEngineV8, EngineConfig  # noqa: E402
from engine.gpu.manager import GPUManager  # noqa: E402
from engine.gpu.async_ops import AsyncGPUExecutor  # noqa: E402
from engine.gpu.embeddings import GPUEmbeddingCache  # noqa: E402
from engine.gpu.batch_processor import GPUBatchProcessor  # noqa: E402
from engine.gpu.memory_optimization import MemoryOptimizer  # noqa: E402
from engine.gpu.parallelization import MultiGPURouter  # noqa: E402
...
async def demo_memory_optimization(gpu_manager: GPUManager):
    ...
    logger.info("  Inference Mode: Optimized")
...
async def demo_batch_processing(gpu_manager: GPUManager):
    ...
    batch_processor = GPUBatchProcessor(gpu_manager=gpu_manager, batch_size=8, max_batch_size=32)

    # Simulate batch processing
    inputs = [f"Sample input {i} for batch processing" for i in range(24)]
...
async def demo_gpu_accelerated_embeddings(gpu_manager: GPUManager):
    ...
    embedding_cache = GPUEmbeddingCache(device=gpu_manager.device, max_cached_embeddings=1000)
    ...
    similarities = embedding_cache.compute_similarity(query_embedding, candidate_embeddings)
...
async def demo_async_gpu_operations(gpu_manager: GPUManager):
    ...
    executor = AsyncGPUExecutor(device=gpu_manager.device, num_streams=4)
    ...
    tasks = [gpu_task(i, 0.1) for i in range(8)]
    ...
    logger.info("  Expected serial time: ~0.8s")
...
async def demo_full_pipeline(gpu_manager: GPUManager):
    ...
    result = await engine.run(text=test_input, context={"domain": "healthcare"}, detail_level=2)
