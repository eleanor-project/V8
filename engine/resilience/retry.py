...
def retry_with_backoff(
    policy: Optional[RetryPolicy] = None, **policy_kwargs
...
async def retry_async(
    func: Callable[..., Awaitable[T]], *args, policy: Optional[RetryPolicy] = None, **kwargs
) -> T:
    """
    ...
    for attempt in range(1, policy.max_attempts + 1):
        try:
            ...
            if attempt == policy.max_attempts:
                logger.error(f"[Retry] {func.__name__} failed after {attempt} attempts: {e}")
                raise

            delay = min(
                policy.initial_delay * (policy.backoff_factor ** (attempt - 1)), policy.max_delay
            )
