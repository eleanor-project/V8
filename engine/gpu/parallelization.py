...
class ModelParallelWrapper:
    ...
    def _apply_data_parallel(self):
        ...
        self.model = nn.DataParallel(
            self.original_model, device_ids=self.device_ids, output_device=self.output_device
        )

        # Move to first device
        self.model = self.model.to(f"cuda:{self.device_ids[0]}")

        logger.info(f"Model parallelized with DataParallel across GPUs: {self.device_ids}")
    ...
    def _apply_pipeline_parallel(self):
        ...
        logger.info(f"Model split with pipeline parallelism across GPUs: {self.device_ids}")
