...
class UncertaintyMeasure(BaseModel):
    ...
    epistemic_gaps: List[str] = Field(
        default_factory=list, description="Specific knowledge gaps identified"
    )
...
class CriticEvaluation(BaseModel):
    ...
    evidence_references: tuple[str, ...] = Field(
        default_factory=tuple, description="Evidence record IDs"
    )
...
class PrecedentAlignment(BaseModel):
    ...
    novel_situation: bool = Field(
        default=False, description="No sufficiently similar precedent exists"
    )
    conflicting_precedents: bool = Field(
        default=False, description="Precedents suggest different outcomes"
    )
    alignment_score: float = Field(
        ..., ge=0.0, le=1.0, description="How well decision aligns with precedent"
    )
...
class DissentRecord(BaseModel):
    ...
    suppression_attempted: bool = Field(
        default=False, description="Flag if aggregation tried to silence dissent"
    )
...
class AggregatedResult(BaseModel):
    ...
    critic_agreement: float = Field(
        ..., ge=0.0, le=1.0, description="Agreement level (low = high disagreement)"
    )
...
class EvidenceRecord(BaseModel):
    ...
    # Record content
    record_type: Literal[
        "critic_evaluation", "escalation", "human_review", "precedent_lookup", "aggregation"
    ]
