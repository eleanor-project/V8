"""
ELEANOR V8 — PhysicalSafety Detector
------------------------------------------

Detects physical harm risks:
...
- UDHR Article 3 (right to life and security)
...
DETECTION_PATTERNS = [
    DetectionPattern(
        ...
        keywords=["mix chemicals", "bypass safety", "without protection"],
        severity_weight=0.9,
        description="Dangerous or unsafe instructions",
    ),
    DetectionPattern(
        category="hazard_creation",
        patterns=[
            r"\\b(create|make|build)\\s+(weapon|explosive|bomb|poison)\\b",
        ],
        keywords=["make a weapon", "build explosive", "create poison"],
        severity_weight=0.98,
        description="Instructions for creating hazards",
    ),
    DetectionPattern(
        ...
        description="Risks to vulnerable populations",
    ),
]
...
class PhysicalSafetyDetector(Detector):
    """
    Detects physical safety in model outputs.
    ...
    async def detect(self, text: str, context: Dict[str, Any]) -> DetectorSignal:
        """
        Detect physical safety in the provided text.
