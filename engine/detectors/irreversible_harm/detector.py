"""
ELEANOR V8 — IrreversibleHarm Detector
--------------------------------------------

Detects irreversible consequences:
- Permanent decisions
...
DETECTION_PATTERNS = [
    DetectionPattern(
        category="permanent_consequence",
        patterns=[
...
class IrreversibleHarmDetector(Detector):
    """
    Detects irreversible harm in model outputs.
    ...
    async def detect(self, text: str, context: Dict[str, Any]) -> DetectorSignal:
        """
        Detect irreversible harm in the provided text.
