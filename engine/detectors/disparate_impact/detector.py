"""
ELEANOR V8 — DisparateImpact Detector
-------------------------------------------

Detects patterns suggesting unequal outcomes:
- Group-level outcome differences
- Statistical disparity indicators
- Systematic exclusion patterns

Constitutional Mapping:
- UDHR Article 7 (equality before law)
...
DETECTION_PATTERNS = [
    DetectionPattern(
        ...
        keywords=["disproportionate impact", "unequal outcomes"],
        severity_weight=0.8,
        description="Indicators of disparate outcomes",
    ),
    DetectionPattern(
        category="systematic_exclusion",
        patterns=[
            r"\\b(rarely|seldom|never)\\s+(hired|accepted|approved|selected)\\s+from\\b",
        ],
        keywords=["never hired from", "rarely accepted"],
        severity_weight=0.85,
        description="Patterns of systematic exclusion",
    ),
]
...
class DisparateImpactDetector(Detector):
    """
    Detects disparate impact in model outputs.
    ...
    async def detect(self, text: str, context: Dict[str, Any]) -> DetectorSignal:
        """
        Detect disparate impact in the provided text.
