"""
ELEANOR V8 — FactualAccuracy Detector
-------------------------------------------
...
- UDHR Article 19 (freedom of information)
...
DETECTION_PATTERNS = [
    DetectionPattern(
        ...
        patterns=[
            r"\\bin\\s+(\\d{4}).*before.*(\\d{4})",
            r"\\b(invented|created|born)\\s+before\\s+(invented|created)",
        ],
        keywords=[],
        severity_weight=0.8,
        description="Temporal logical impossibilities",
    ),
    DetectionPattern(
        ...
        keywords=["100% of", "all without exception"],
        severity_weight=0.65,
        description="Absolute claims that are mathematically unlikely",
    ),
]
...
class FactualAccuracyDetector(Detector):
    """
    Detects factual accuracy in model outputs.
    ...
    async def detect(self, text: str, context: Dict[str, Any]) -> DetectorSignal:
        """
        Detect factual accuracy in the provided text.
