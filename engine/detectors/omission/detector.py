"""
ELEANOR V8 — Omission Detector
------------------------------------

Detects critical information omissions:
- Missing disclosures
...
DETECTION_PATTERNS = [
    DetectionPattern(
        ...
        keywords=["failed to disclose", "omitted information"],
        severity_weight=0.75,
        description="Missing critical disclosures",
    ),
    DetectionPattern(
        ...
        keywords=["hidden risk", "undisclosed danger"],
        severity_weight=0.8,
        description="Hidden or undisclosed risks",
    ),
]
...
class OmissionDetector(Detector):
    """
    Detects omission in model outputs.
    ...
    async def detect(self, text: str, context: Dict[str, Any]) -> DetectorSignal:
        """
        Detect omission in the provided text.
