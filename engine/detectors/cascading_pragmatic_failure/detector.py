"""
ELEANOR V8 — CascadingPragmaticFailure Detector
-----------------------------------------------------

Detects cascading practical failures:
- Sequential implementation failures
- Compound pragmatic risks
- Multi-stage breakdown

Constitutional Mapping:
- UNESCO AI Ethics (continuous assessment)
"""
...
DETECTION_PATTERNS = [
    DetectionPattern(
        category="sequential_failure",
        patterns=[
            r"\\b(one .* leads to another|failure begets failure)\\b",
            r"\\b(compound|cumulative|cascading)\\s+(failures?|problems?|issues?)\\b",
        ],
        keywords=["cascading failures", "compound problems"],
        severity_weight=0.75,
        description="Sequential or cascading failures",
    ),
    DetectionPattern(
        category="implementation_breakdown",
        patterns=[
            r"\\b(each step|every stage)\\s+(fails|breaks down|encounters issues)\\b",
        ],
        keywords=["multi-stage breakdown"],
        severity_weight=0.7,
        description="Multi-stage implementation breakdown",
    ),
]
...
class CascadingPragmaticFailureDetector(Detector):
    """
    Detects cascading pragmatic failure in model outputs.
    ...
    def __init__(self):
        self.name = "cascading_pragmatic_failure"
        self.version = "8.0"
    ...
    async def detect(self, text: str, context: Dict[str, Any]) -> DetectorSignal:
        """
        Detect cascading pragmatic failure in the provided text.
