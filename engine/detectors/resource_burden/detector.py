"""
ELEANOR V8 — ResourceBurden Detector
------------------------------------------

Detects excessive resource requirements:
- Hidden costs
- Scalability issues
- Maintenance burden
...
- UNESCO AI Ethics (proportionality)
"""
...
DETECTION_PATTERNS = [
    DetectionPattern(
        ...
        keywords=["hidden fees", "extra costs", "ongoing charges"],
        severity_weight=0.65,
        description="Hidden or undisclosed costs",
    ),
    DetectionPattern(
        ...
        keywords=["won't scale", "doesn't work for large"],
        severity_weight=0.7,
        description="Scalability limitations",
    ),
]
...
class ResourceBurdenDetector(Detector):
    """
    Detects resource burden in model outputs.
    ...
    async def detect(self, text: str, context: Dict[str, Any]) -> DetectorSignal:
        """
        Detect resource burden in the provided text.
