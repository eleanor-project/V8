...
class CacheManager:
    ...
    def __init__(
        ...
        for prefix in self.l1_sizes:
            self.l1_caches[prefix] = TTLCache(
                maxsize=self.l1_sizes[prefix], ttl=self.l1_ttls[prefix]
            )
    ...
    def _get_l1(self, prefix: str) -> TTLCache:
        ...
        if prefix not in self.l1_caches:
            self.l1_caches[prefix] = TTLCache(
                maxsize=self.l1_sizes.get(prefix, 1000), ttl=self.l1_ttls.get(prefix, 300)
            )
    ...
    async def _redis_setex(self, key: str, ttl: int, value: str) -> None:
        ...
        if hasattr(self.redis, "setex"):
            ...
            # redis-py style (sync)
            await asyncio.get_event_loop().run_in_executor(None, self.redis.setex, key, ttl, value)

    def get_stats(self) -> Dict[str, Dict[str, Any]]:
        """Get cache statistics for all prefixes."""
        return {prefix: stats.to_dict() for prefix, stats in self.stats.items()}
