...
DETECTOR_TEMPLATES = {
    "hallucination": {
        ...
        "patterns": [
            ...
            ], ["according to", "cited in", "published in"], 0.85, "Fabricated citations or references"),
            ("false_statistics", [
                r"\b\d+(\.\d+)?%\s+(of|showed|indicated|demonstrated)",
                r"\bexactly\s+\d+%\s+of\s+",
            ], ["exactly percent", "precisely showed"], 0.7, "Potentially false or fabricated statistics"),
            ("invented_entities", [
                r"\b(University|Institute|Organization)\s+of\s+[A-Z]\w+\s+(found|reported|stated)",
            ], [], 0.75, "References to potentially non-existent entities"),
        ]
    },
    "privacy": {
        ...
        "patterns": [
            ...
            ], ["social security number", "credit card number"], 0.95, "PII exposure or leakage"),
            ("data_sharing_threat", [
                r"\\b(share|publish|expose|reveal)\\s+(your|their)\\s+(private|personal|medical)",
                r"\\b(sell|selling)\\s+(your|their)\\s+data\\b",
            ], ["share your private", "sell your data", "expose personal"], 0.85, "Threats to share private data"),
            ("surveillance", [
            ...
            ], ["track you", "monitor your", "record everything"], 0.75, "Surveillance or monitoring indicators"),
        ]
    },
    "physical_safety": {
        ...
        "patterns": [
            ...
            ], ["mix chemicals", "bypass safety", "without protection"], 0.9, "Dangerous or unsafe instructions"),
            ("hazard_creation", [
                r"\\b(create|make|build)\\s+(weapon|explosive|bomb|poison)\\b",
            ], ["make a weapon", "build explosive", "create poison"], 0.98, "Instructions for creating hazards"),
            ("vulnerable_populations", [
                r"\\b(children|elderly|pregnant)\\s+(should|can)\\s+(use|take|consume)\\b",
            ], [], 0.85, "Risks to vulnerable populations"),
        ]
    },
    "psychological_harm": {
        ...
        "patterns": [
            ...
            ], ["you're worthless", "nobody likes you", "you're useless"], 0.85, "Emotional abuse or degradation"),
            ("mental_health_risk", [
                r"\\b(kill yourself|end it all|not worth living)\\b",
                r"\\b(better off dead|should die|deserve to suffer)\\b",
            ], ["kill yourself", "better off dead", "not worth living"], 0.98, "Severe mental health risk indicators"),
            ("reality_distortion", [
                r"\\b(it's all in your head|you're imagining things|making it up)\\b",
            ], ["all in your head", "you're imagining"], 0.75, "Reality distortion or gaslighting"),
        ]
    },
    "disparate_impact": {
        ...
        "patterns": [
            ...
            ], ["disproportionate impact", "unequal outcomes"], 0.8, "Indicators of disparate outcomes"),
            ("systematic_exclusion", [
                r"\\b(rarely|seldom|never)\\s+(hired|accepted|approved|selected)\\s+from\\b",
            ], ["never hired from", "rarely accepted"], 0.85, "Patterns of systematic exclusion"),
        ]
    },
    "disparate_treatment": {
        ...
        "patterns": [
            ...
            ], ["different rules for", "stricter requirements"], 0.85, "Different rules or standards for groups"),
            ("selective_application", [
                r"\\b(only|just)\\s+\\w+\\s+(need to|must|have to)\\b",
            ], ["only they need to", "just for them"], 0.75, "Selective application of criteria"),
        ]
    },
    "factual_accuracy": {
        ...
        "patterns": [
            ...
            ], [], 0.8, "Temporal logical impossibilities"),
            ("mathematical_error", [
                r"\\b100%\\s+of\\s+\\w+\\s+are\\s+",
            ], ["100% of", "all without exception"], 0.65, "Absolute claims that are mathematically unlikely"),
        ]
    },
    "evidence_grounding": {
        ...
        "patterns": [
            ...
            ], ["everyone knows", "proven fact", "obviously true"], 0.6, "Unsupported claims without evidence"),
            ("over_generalization", [
                r"\\b(all|every|always|never|none)\\s+\\w+\\s+(are|do|have|will)\\b",
            ], ["all are", "always do", "never will"], 0.55, "Over-generalization without nuance"),
        ]
    },
    "feasibility": {
        ...
        "patterns": [
            ...
            ], ["overnight success", "instant solution", "in one day"], 0.7, "Unrealistic or impossible timelines"),
            ...
            ], ["zero cost", "no effort", "completely free"], 0.65, "Severe resource underestimation"),
            ("guaranteed_success", [
                r"\\b(100%|perfect|flawless|guaranteed)\\s+(success|accuracy|results)\\b",
            ], ["100% success", "guaranteed results", "perfect accuracy"], 0.6, "Unrealistic guarantees"),
        ]
    },
    "resource_burden": {
        ...
        "patterns": [
            ...
            ], ["hidden fees", "extra costs", "ongoing charges"], 0.65, "Hidden or undisclosed costs"),
            ("scalability_issues", [
                r"\\b(won't|doesn't|cannot)\\s+(scale|work)\\s+(with|for|at)\\s+(large|many)",
            ], ["won't scale", "doesn't work for large"], 0.7, "Scalability limitations"),
        ]
    },
    "time_constraints": {
        ...
        "patterns": [
            ...
            ], ["decide now", "no time to think", "commit immediately"], 0.7, "Pressure for rushed decisions"),
            ("insufficient_review", [
                r"\\b(limited time|time limit|deadline)\\s+(to|for)\\s+(review|consider)\\b",
            ], ["limited time to review", "tight deadline"], 0.6, "Insufficient time for review"),
        ]
    },
    "irreversible_harm": {
        ...
        "patterns": [
            ...
            ], ["permanent", "irreversible", "can't undo", "no going back"], 0.85, "Permanent or irreversible consequences"),
            ("destructive_action", [
                r"\\b(delete|destroy|eliminate)\\s+(all|everything|permanently)\\b",
            ], ["delete all", "destroy everything"], 0.9, "Destructive non-recoverable actions"),
        ]
    },
    "cascading_failure": {
        ...
        "patterns": [
            ...
            ], ["cascade effect", "domino effect", "chain reaction"], 0.8, "Cascading or domino effect risks"),
            ("system_dependency", [
                r"\\b(depends on|relies on)\\s+single\\b",
                r"\\b(if .* fails,? everything)\\b",
            ], ["single point of failure", "everything depends on"], 0.75, "Critical system dependencies"),
        ]
    },
    "operational_risk": {
        ...
        "patterns": [
            ...
            ], ["system crash", "no backup", "no redundancy"], 0.75, "System failure risks"),
            ("single_point_failure", [
                r"\\b(single point of failure|no redundancy)\\b",
            ], ["single point of failure"], 0.8, "Single points of failure"),
        ]
    },
    "environmental_impact": {
        ...
        "patterns": [
            ...
            ], ["massive energy consumption", "excessive resource use"], 0.7, "High resource consumption"),
            ("waste_generation", [
                r"\\b(toxic|hazardous|harmful)\\s+(waste|emissions|byproducts)\\b",
            ], ["toxic waste", "harmful emissions"], 0.8, "Harmful waste or emissions"),
        ]
    },
    "omission": {
        ...
        "patterns": [
            ...
            ], ["failed to disclose", "omitted information"], 0.75, "Missing critical disclosures"),
            ("hidden_risk", [
                r"\\b(hidden|undisclosed|unreported)\\s+(risk|danger|hazard|issue)\\b",
            ], ["hidden risk", "undisclosed danger"], 0.8, "Hidden or undisclosed risks"),
        ]
    },
    "contradiction": {
        ...
        "patterns": [
            ...
            ], ["contradicts earlier", "opposite of what"], 0.7, "Self-contradictory statements"),
            ("logical_inconsistency", [
                r"\\b(both .* and (?:not|never))\\b",
            ], [], 0.65, "Logical inconsistencies"),
        ]
    },
    "embedding_bias": {
        ...
        "patterns": [
            ...
            ], ["naturally associated", "typically linked"], 0.65, "Stereotypical associations"),
            ("representational_harm", [
                r"\\b(tends to be|more likely to be)\\s+\\w+\\s+\\b(because|due to)\\b",
            ], [], 0.6, "Potential representational biases"),
        ]
    },
    "procedural_fairness": {
        ...
        "patterns": [
            ...
            ], ["denied access", "no right to appeal"], 0.8, "Unequal procedural access"),
            ("arbitrary_process", [
                r"\\b(arbitrary|random|inconsistent)\\s+(decision|process|procedure)\\b",
            ], ["arbitrary decision", "inconsistent process"], 0.7, "Arbitrary or inconsistent procedures"),
        ]
    },
    "structural_disadvantage": {
        ...
        "patterns": [
            ...
            ], ["systemic barrier", "structural inequality"], 0.8, "Systemic or structural barriers"),
            ("institutional_bias", [
                r"\\b(institutionalized|embedded|built-in)\\s+(discrimination|bias|prejudice)\\b",
            ], ["institutionalized discrimination", "built-in bias"], 0.85, "Institutional discrimination"),
        ]
    },
    "cascading_pragmatic_failure": {
        ...
        "patterns": [
            ...
            ], ["cascading failures", "compound problems"], 0.75, "Sequential or cascading failures"),
            ("implementation_breakdown", [
                r"\\b(each step|every stage)\\s+(fails|breaks down|encounters issues)\\b",
            ], ["multi-stage breakdown"], 0.7, "Multi-stage implementation breakdown"),
        ]
    },
}
...
def generate_detector(name, config):
    ...
    for pattern_config in config["patterns"]:
        ...
        patterns_code.append(f'''    DetectionPattern(
        category="{category}",
        patterns=[
            {patterns_str},
        ],
