...
- The local stack now defines a `dependency-exporter` service plus a Prometheus scrape job (`docker/docker-compose.yaml` + `docker/prometheus.yml`) so the exporter runs automatically next to Grafana/Prometheus.
